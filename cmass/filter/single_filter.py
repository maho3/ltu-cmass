...
def get_filter(filter_name):
    ...
    return importlib.import_module(f'.filter_lib.{filter_name}',package='cmass.filter').filter
...
def main(cfg: DictConfig) -> None:
    ...
    np.save(pjoin(source_path, 'obs/filtered', f'rdz{cfg.bias.hod.seed}_{cfg.filter.filter_name}.npy'), filtered_rdz)
    np.save(pjoin(source_path, 'obs/filtered', f'rdz{cfg.bias.hod.seed}_{cfg.filter.filter_name}_weight.npy'), weight)
