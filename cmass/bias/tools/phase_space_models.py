...
class Satellites_vBiasedNFWPhaseSpace(BiasedNFWPhaseSpace): 
    ...
    def mc_vel(self, table, seed=None):
        ...
        table['vx'][:] += vx * self.param_dict['eta_vb.satellites']
        table['vy'][:] += vy * self.param_dict['eta_vb.satellites']
        table['vz'][:] += vz * self.param_dict['eta_vb.satellites']
    ...
    def calculate_conc_gal_bias(self, seed=None, **kwargs):
        ...
        result = np.full_like(mass, self.param_dict['conc_gal_bias.satellites'])
