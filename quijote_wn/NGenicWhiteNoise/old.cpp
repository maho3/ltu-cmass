...
int main(int argc, char ** argv) {
    ...
    const std::uint64_t num_mesh = num_mesh_1d * num_mesh_1d * num_mesh_1d;
    const std::uint64_t num_mesh_fftw = 2 * num_modes;
    ...
    std::vector<double> white_noise(num_mesh_fftw);
    #pragma omp parallel
    {
        TaskDistributor tasks(white_noise.size());
        std::fill(white_noise.begin() + tasks.start, white_noise.begin() + tasks.stop, 0);
    }

    // *** Draw white noise modes in Fourier space
    {
        std::complex<double> * white_noise_modes = reinterpret_cast<std::complex<double> *>(white_noise.data());
        #pragma omp parallel
        {
            ...
            for (i = 0; i < num_mesh_1d; i++) {
                for (j = 0; j < num_mesh_1d; j++) {
                    ...
                    for (k = 0; k < half_num_mesh_1d; k++) {
                        ...
                        while (amplitude == 0.);
                        amplitude = 4. * sqrt(-log(amplitude));
    ...
    // *** Inverse DFT transform, reformat without DFT buffer, and write to file
    timer.printStart("Inverse Fourier transforming...");
    {
        const double norm = sqrt(2 * num_mesh);
        transformInverseDFT3D(white_noise, norm);
    }
    timer.printDone();
    timer.printStart("Saving...");
    // unformatDFTMesh(white_noise);
    writeVector<double, double>(white_noise_filename, white_noise);
    timer.printDone();
