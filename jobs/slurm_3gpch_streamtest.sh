#!/bin/bash
#SBATCH --job-name=3gpch_stream # Job name
...
# Preserve the baseline (non-streaming) outputs for regression diff,
# and clear the way so fastpm re-runs this lhid with streaming enabled.
if [ -f "$outdir/$lhid/nbody.h5" ] && [ ! -f "$outdir/$lhid/nbody_baseline.h5" ]; then
    mv "$outdir/$lhid/nbody.h5" "$outdir/$lhid/nbody_baseline.h5"
    mv "$outdir/$lhid/config.yaml" "$outdir/$lhid/config_baseline.yaml" 2>/dev/null
fi

for offset in 0; do
    loff=$((lhid + offset))
    postfix="nbody=$nbody sim=$sim nbody.lhid=$loff multisnapshot=$multisnapshot $extras"
    echo "Running FastPM with streaming postprocess for lhid=$loff"
